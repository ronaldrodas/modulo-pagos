...
const nextMetaRoot = path.normalize(path.join(process.cwd(), BUILD_OUTPUT_DIRECTORY));
if (!nextMetaRoot.startsWith(process.cwd())) {
  console.error("Invalid build output directory.");
  process.exit(1);
}
...
function getScriptSize(scriptPath) {
  const encoding = "utf8";
  const p = path.normalize(path.join(nextMetaRoot, scriptPath));
  
  if (!p.startsWith(nextMetaRoot)) {
